...
const Publicite = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://185.98.139.246:9090/ogatemanagement-api/admin/supprimerpublicite/${id}`);
  ...
  return (
   ...
   <div className="w-full py-8 px-5">
          <h1 className="text-3xl text-black font-bold">Publicité</h1>
      <div className="flex justify-end mb-4">
        <button className="bg-primary hover:opacity-70 text-white font-bold py-2 px-4 rounded mr-2" onClick={() => setShowForm(!showForm)}>
          Ajouter
        </button>
      </div>
  ...
  (Array.isArray(data) && data.length > 0 ? (
    data.map((item, index) => (
      <tr key={index} className="bg-gray-100">
        ...
        <td className="border px-4 py-2 text-center" >
          <button onClick={() => handleEdit(item)} className=" hover:bg-green-700 hover:text-white text-black font-bold py-1 px-2 rounded mr-2">
            <FaEdit />
          </button>
          <button onClick={() => handleDelete(item.id)} className="hover:bg-red-600 hover:text-white text-black font-bold py-1 px-2 rounded">
            <FaTrash />
          </button>
        </td>
      </tr>
    ...
    <tr>
      ...
      {showForm && (
        <>
          ...
          <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-50 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
              ...
              <input id="dropzone-file" type="file" className="hidden" onChange={handleChange} />
                ...
                <button type="submit" className="bg-primary hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2">Soumettre</button>
              </form>
            </div>
          </dialog>
        </>
       ...
       {showEditForm && (
        <>
          ...
          <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-50 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
              ...
              <input id="dropzone-file" type="file" className="hidden" onChange={handleEditChange} />
          </label>
        </div>
      ...
      <div className="flex justify-center mt-4">
        <button onClick={() => setPage(page > 0 ? page - 1 : page)} className="bg-primary hover:scale-110 active:bg-brand-700 dark:bg-brand-400 dark:hover:bg-brand-300 dark:active:bg-brand-200 flex items-center justify-center rounded-full p-2 text-1xl text-white transition duration-200 hover:cursor-pointer dark:text-white">
        <MdChevronLeft/>
        </button>
        <span className="text-2xl">{page + 1}</span>
        <button onClick={() => setPage(page + 1)} className="bg-primary hover:scale-110 active:bg-brand-700 dark:bg-brand-400 dark:hover:bg-brand-300 dark:active:bg-brand-200 flex items-center justify-center rounded-full p-2 text-1xl text-white transition duration-200 hover:cursor-pointer dark:text-white">
        <MdChevronRight />
        </button>
      </div>
    </div>
